...
void InvoiceManagementWidget::setupActionsMenu()
{
    ...
    // Add button to your layout (replace with your actual top layout)
...
void InvoiceManagementWidget::setupConnections() {
    ...
    QPushButton *testButton = new QPushButton("Test Reminders", this);
    connect(testButton, &QPushButton::clicked, this, &InvoiceManagementWidget::testReminderSystem);
    ui->verticalLayout->addWidget(testButton); // Add to your layout
                                                ...
                                                const QString &reminderType)
{
    ...
    qDebug() << "Would send reminder email to:" << recipientEmail
...
void InvoiceManagementWidget::checkAndSendReminders()
{
    ...
    int preDueDays = 3; // Send reminder 3 days before due date

    // 1. Check for pre-due reminders
    ...
    preDueQuery.bindValue(":reminderDate", today.addDays(preDueDays));
    preDueQuery.bindValue(":today", today);

    if (preDueQuery.exec()) {
        while (preDueQuery.next()) {
            int invoiceId = preDueQuery.value(0).toInt();
            QString invoiceNumber = preDueQuery.value(1).toString();
            QDate dueDate = preDueQuery.value(2).toDate();
            double amount = preDueQuery.value(3).toDouble();
            QString firstName = preDueQuery.value(4).toString();
            QString lastName = preDueQuery.value(5).toString();
            QString email = preDueQuery.value(6).toString();
            QString clientName = QString("%1 %2").arg(firstName, lastName).trimmed();

            QString body = m_emailService->generateReminderContent(
                EmailService::PreDueReminder, clientName, invoiceNumber, dueDate, amount);

            if (m_emailService->sendEmail(email,
                                          QString("Upcoming Payment Due for Invoice #%1").arg(invoiceNumber), body)) {

                // Update last reminder date
                QSqlQuery updateQuery(db);
                updateQuery.prepare(
                    "UPDATE INVOICES SET LAST_REMINDER_DATE = :today, "
                    "REMINDER_COUNT = REMINDER_COUNT + 1 "
                    "WHERE INVOICE_ID = :invoiceId"
                    );
                updateQuery.bindValue(":today", today);
                updateQuery.bindValue(":invoiceId", invoiceId);
                updateQuery.exec();
            }
        }
    }

    // 2. Check for due date reminders (similar structure)
    // 3. Check for overdue reminders (similar structure)
}
...
void InvoiceManagementWidget::sendPaymentVerification(int invoiceId)
{
    ...
    query.prepare(
        ...
        "AND i.PAYMENT_VERIFICATION_SENT = 0"
        );
    ...
    if (query.exec() && query.next()) {
        ...
        QString body = m_emailService->generateReminderContent(
            EmailService::PaymentVerification, clientName, invoiceNumber, QDate(), amount);

        if (m_emailService->sendEmail(email,
                                      QString("Payment Received for Invoice #%1").arg(invoiceNumber), body)) {
...
{
    // Replace with your actual email provider details
    m_smtpServer = "hassinerahma@gmail.com";
    m_smtpPort = 587;
    m_senderEmail = "hassinerahma@gmail.com";
    m_senderPassword = "123"; // Consider using app password
}
/*
bool EmailService::sendEmail(const QString &recipient, const QString &subject, const QString &body)
{
    // Implement actual email sending using your preferred method
    // This is a placeholder - you might use SMTP, a web API, etc.

    try{
        qDebug() << "Sending email to:" << recipient;
        qDebug() << "Subject:" << subject;
        qDebug() << "Body:" << body;

        qInfo() << "Email sent successfully to:" << recipient;
        return true;
    } catch (const std::exception &e) {
        qCritical() << "Failed to send email to" << recipient << ":" << e.what();
        return false;
    }

}*/
// In emailservice.cpp, modify the sendEmail function for testing:
bool EmailService::sendEmail(const QString &recipient, const QString &subject, const QString &body)
{
    // For testing, just log to console and files
    QString logMessage = QString("Would send email to: %1\nSubject: %2\nBody:\n%3\n")
                             .arg(recipient)
                             .arg(subject)
                             .arg(body);
                                              ...
                                              int daysOverdue)
{
    ...
    switch(type) {
    case PreDueReminder:
        subject = QString("Upcoming Payment Due for Invoice #%1").arg(invoiceNumber);
        body = QString("Dear %1,\n\n"
                       "This is a friendly reminder that payment for invoice #%2 "
                       "in the amount of %3 is due on %4.\n\n"
                       "Please ensure payment is submitted by the due date.\n\n"
                       "Thank you,\n"
                       "Your Billing Team").arg(clientName, invoiceNumber,
                        formattedAmount, dueDate.toString("MMMM d, yyyy"));
        break;

    case DueDateReminder:
        subject = QString("Payment Due Today for Invoice #%1").arg(invoiceNumber);
        body = QString("Dear %1,\n\n"
                       "This is a reminder that payment for invoice #%2 "
                       "in the amount of %3 is due today (%4).\n\n"
    ...
    case OverdueReminder:
        subject = QString("Overdue Payment for Invoice #%1").arg(invoiceNumber);
        body = QString("Dear %1,\n\n"
                       "Our records indicate that payment for invoice #%2 "
                       "in the amount of %3 is overdue by %4 days (due date: %5).\n\n"
                       "Please submit payment immediately to avoid any late fees or "
                       "service interruptions.\n\n"
                       "Thank you,\n"
                       "Your Billing Team").arg(clientName, invoiceNumber,
                        formattedAmount, QString::number(daysOverdue),
                        dueDate.toString("MMMM d, yyyy"));
        break;

    case PaymentVerification:
        subject = QString("Payment Received for Invoice #%1").arg(invoiceNumber);
        body = QString("Dear %1,\n\n"
                       "We have received your payment of %2 for invoice #%3.\n\n"
                       "Thank you for your prompt payment!\n\n"
                       "Sincerely,\n"
                       "Your Billing Team").arg(clientName, formattedAmount, invoiceNumber);
        break;
    }
...
void InvoiceManagementWidget::testReminderSystem()
{
    qDebug() << "=== STARTING REMINDER SYSTEM TEST ===";
    ...
    int testInvoiceId = ensureTestInvoiceExists();
    if (testInvoiceId <= 0) {
        qWarning() << "Failed to create test invoice!";
        return;
    }

    QSqlDatabase db = QSqlDatabase::database();

    // Test 1: Pre-due reminder (due tomorrow)
    qDebug() << "TEST 1: Pre-due reminder (due tomorrow)";
    setTestInvoiceDueDate(db, testInvoiceId, QDate::currentDate().addDays(1));
    checkAndSendReminders();

    // Test 2: Due today reminder
    qDebug() << "TEST 2: Due today reminder";
    setTestInvoiceDueDate(db, testInvoiceId, QDate::currentDate());
    checkAndSendReminders();

    // Test 3: Overdue reminder (5 days overdue)
    qDebug() << "TEST 3: Overdue reminder";
    setTestInvoiceDueDate(db, testInvoiceId, QDate::currentDate().addDays(-5));
    checkAndSendReminders();

    qDebug() << "=== REMINDER SYSTEM TEST COMPLETE ===";
    QMessageBox::information(this, "Test Complete",
                             "Reminder tests executed. Check email_log.txt for results.");
}
