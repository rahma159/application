    ...
    ui(new Ui::ClientPaymentWidget) // Create the UI instance from Designer
{
    ...
    this->setObjectName("ClientPaymentWidget");
    ...
    QFile styleFile(":/styles/client_payment_style.qss"); // Path from .qrc alias
    if (styleFile.open(QFile::ReadOnly | QFile::Text)) {
        QTextStream stream(&styleFile);
        QString styleSheetContent = stream.readAll();
        this->setStyleSheet(styleSheetContent); // Apply stylesheet to this widget
        styleFile.close();
        qDebug() << "ClientPaymentWidget: Stylesheet loaded successfully.";
    } else {
        qWarning() << "ClientPaymentWidget: Could not load stylesheet. File:" << styleFile.fileName() << "Error:" << styleFile.errorString();
    }
} // End Constructor
